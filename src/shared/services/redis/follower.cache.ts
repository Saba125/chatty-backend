...
export class FollowerCache extends BaseCache {
  ...
  public async updateBlockedUserPropInCache(key: string, props:string,value:string, type: 'block' | 'unblock'): Promise<void> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }
      const response:string = await this.client.HGET(`users:${key}`, props) as string;
      ...
      const dataToSave: string[] = [`${props}`,JSON.stringify(blocked)]
