import { IPostDocument, IReactions, ISavePostToCache } from '@root/features/posts/interfaces/post.interface';
...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    const { key, currentUserId, createdPost, uId } = data;
    const {
      _id,
      userId,
      username,
  ...
  public async getTotalPostsFromUser(uId: number): Promise<number> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }
